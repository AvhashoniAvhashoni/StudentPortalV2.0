...
export class EnrollPage implements OnInit {
    ...
    uploadID(event) {
        this.id = <File> event.target.files[0];
        this.presentLoadingWithOptions();
        this._service.upload("idCard/", this.user.id, this.id).then(res => {
    ...
    uploadProofOfRes(event) {
        ...
        this.presentLoadingWithOptions();
        this._service.upload("cv/", this.user.id, this.cv).then(res => {
            res.ref.getDownloadURL().then(url => {
                this.cvUrl = url;
                this._loadingController.dismiss().then(res => {
                });
    ...
    uploadResults(event) {
        this.results = <File> event.target.files[0];
        this.presentLoadingWithOptions();
        this._service.upload("results/", this.user.id, this.results).then(res => {
            res.ref.getDownloadURL().then(url => {
                this.resultsUrl = url;
                this._loadingController.dismiss().then(res => {
                });
    ...
    uploadProofOfPay(event) {
        this.pop = <File> event.target.files[0];
        this.presentLoadingWithOptions();
        this._service.upload("proofOfPay/", this.user.id + "/" + new Date().toString(), this.pop).then(res => {
            res.ref.getDownloadURL().then(url => {
